...
author: yuanyxh
imageUrl: https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-logo.webp
...
![讨论图 1](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-01.webp)

![讨论图 2](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-02.webp)

![讨论图 3](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-03.webp)

讨论后得出的结果是 vuepress 将滚动位置保存在了 `history` 对象的 `state` 属性中，因为历史记录在刷新后是不会变的，所以才可以实现保存数据的功能，而不同的页签历史记录是不同的，也就可以解释为什么新开页签的页面不会有滚动恢复。

![控制台打印滚动位置](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-04.webp)

## 溯源

有了线索我们就可以有针对性的去查找，我下载了 vuepress 的源码，针对路由部分进行搜索，找到了以下代码：

![vuepress 有关路由的代码](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-05.webp)
...
![vue-router pushWithRedirect 函数签名的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-06.webp)

`pushWithRedirect` 又调用了 `handleScroll`，在这里获取了保存的滚动位置：

![保存滚动位置代码的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-07.webp)
...
![finalizeNavigation 方法代码的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/record_vuepress_rolling_recovery-08.webp)
