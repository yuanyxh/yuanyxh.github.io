...
author: yuanyxh
imageUrl: https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-logo.webp
...
![展示加密参数的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-01.webp)

![展示加密参数的图片2](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-02.webp)
...
![如何在 chrome 中保存完整请求的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-03.webp)

## 确认 encryptId 生成规则

在浏览器中要确认一个请求参数是如何生成的，最好的方式就是 xhr/fetch 断点，并沿调用堆栈向前追溯，我们使用 sources 面板的 xhr/fetch breakpoints 功能，添加需要断点的请求 url：

![如何在 chrome 添加网络断点的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-04.webp)

然后再次发布文章，就会发现代码执行被断在了对应的 xhr/fetch 请求上面，如下图：

![chrome 网络请求断点成功的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-05.webp)

然后就可以沿着调用堆栈进行代码分析了，在这里发现 encryptId 来源于另一个接口：

![加密参数 encryptId 接口的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-06.webp)
...
![chrome 元素面板对应的元素树图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-07.webp)
...
![元素树中的 quill 元素图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-08.webp)

![生成 html 后替换到 boss 上的效果图](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-09.webp)

![生成 html 后替换到 boss 上的效果图2](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-10.webp)
...
![chrome 网络请求头图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-11.webp)

![chrome 网络请求头图片2](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-12.webp)

## 图片转存

到这里基本需要的参数都准备完成了，但如果就这样编写接口的话，你会发现返回的请求信息是

![接口请求失败的图片](https://image-gallery-blog.oss-cn-shenzhen.aliyuncs.com/blog/boss_article_auto_upload-13.webp)
